...
function GenerateForm({ searchParams }: { searchParams: URLSearchParams }) {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      // Restore form state if user just logged in
      if (user) {
        const savedState = localStorage.getItem('generate_form_state');
        if (savedState) {
          try {
            const { params: savedParams, customInputs: savedCustomInputs, timestamp } = JSON.parse(savedState);
            
            // Only restore if saved within last 30 minutes
            if (Date.now() - timestamp < 30 * 60 * 1000) {
              setParams(savedParams);
              setCustomInputs(savedCustomInputs);
              toast.success('Form data restored! You can continue where you left off.');
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    setLoadingStep('Preparing your request...');
    ...
    try {
      ...
      // 检查用户是否登录
      if (!user) {
        throw new Error('Please sign in to generate lyrics');
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 && data.action === 'upgrade') {
