...
// 生成静态参�?export async function generateStaticParams() {
...
async function getOtherCategories(currentCategoryId: number): Promise<Category[]> {
  // 首先尝试从缓存获�?  const categories = await cacheService.getCategories();
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  if (!category) {
    // 友好降级：分类不存在时，返回 200 并给出提示，避免因重定向回到一�?404 页被误判为“登录失败�?    return (
...
// 启用动态路�?- 允许访问未预生成的分类页�?export const dynamicParams = false;
