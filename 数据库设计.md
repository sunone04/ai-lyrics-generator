...
- **扩展性**：支持分页、搜索、统计等高级功能。
- **个人风格功能**：支持高级会员上传个人歌词样本，AI生成时参考用户风格，提升个性化体验。

## 核心功能概述

### 个人风格系统
- **用途**: 允许高级会员上传最多5首个人创作的歌词作为AI训练样本
- **限制**: 每首歌词最多500字符，标题最多100字符
- **安全**: 通过RLS确保用户只能访问自己的数据
- **集成**: 在歌词生成时可选择个人风格作为参考，AI会学习用户的创作风格
...
| `personal_style_id` | `BIGINT` | **外键 (Foreign Key)**，关联到 `public.personal_styles.id`。如果用户选择了个人风格作为参考，则记录该风格ID。可选。 |
...
- **用途**: 存储用户的个人风格歌词样本，用于AI生成时参考用户的创作风格。仅限高级会员使用。
- **列定义**:
| 列名 | 数据类型 | 描述与约束 |
| :--- | :--- | :--- |
| `id` | `BIGINT` | **主键 (Primary Key)**。自增ID。 |
| `user_id` | `UUID` | **外键 (Foreign Key)**，关联到 `public.profiles.id`。`NOT NULL`。 |
| `title` | `TEXT` | 歌词标题。`NOT NULL`，最大100字符。 |
| `lyrics` | `TEXT` | 歌词内容。`NOT NULL`，最大500字符。 |
| `music_style` | `TEXT` | 音乐风格（如 Pop、Rock、Hip-hop 等）。可选。 |
| `language` | `TEXT` | 歌词语言。默认为 'English'。 |
| `word_count` | `INTEGER` | 歌词字数统计。`NOT NULL`。 |
| `created_at` | `TIMESTAMPTZ` | 创建时间。默认为 `NOW()`。 |
| `updated_at` | `TIMESTAMPTZ` | 最后更新时间。默认为 `NOW()`。 |

**约束与策略**:
- **字符长度约束**: `max_lyrics_length` (歌词最大500字符), `max_title_length` (标题最大100字符)
- **行级安全策略 (RLS)**: 用户只能访问自己的个人风格数据
- **用户数量限制**: 每个用户最多只能创建5首个人风格（通过触发器 `check_user_style_limit_trigger` 实现）
- **字数统计**: 在应用层自动计算并存储歌词字数
